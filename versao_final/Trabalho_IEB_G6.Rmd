...
knitr::opts_chunk$set(fig.width=7, fig.height=5, out.width='0.6\\textwidth', fig.align='center')
```
...
```{r fig6, echo=FALSE,warning=FALSE, fig.cap="Gráfico de Correlação",  fig.pos='H'}
...
\begin{align*}
Taxa\_Desemprego_t &= \beta_0 - \beta_1 \cdot PIB\_Real_t + \beta_2 \cdot Proporção\_15\_24_t\\
&\quad - \beta_3 \cdot Emprego\_Terciário_t - \beta_4 \cdot Rendimento\_Médio\_Líquido_t
\end{align*}

,onde

- $\beta_0$ é o termo independente do modelo,
...
modelo_bayes <- stan_glm(
  ...
  prior_intercept = normal(media_desemprego,8 ),   # prioiri para intercepto
  chains = 4,                                       
  iter = 4000,                       
  warmup = 1000,                     
  seed = 123
...
# Posterior Predictive Check e Avaliação do Ajustamento

Antes de proceder à avaliação e seleção formal de modelos Bayesianos, foi realizada uma análise de posterior predictive checks (PPC) com o objetivo de avaliar a adequação do modelo completo aos dados observados. O posterior predictive check consiste na comparação entre os dados observados e dados simulados a partir da distribuição preditiva posterior do modelo. Se o
modelo estiver bem ajustado, espera-se que as observações reais sejam compatíveis com as distribuições simuladas, não apresentando padrões sistemáticos de desvio.

```{r fig11, echo=FALSE, fig.cap="Gráfico do PPC", fig.pos='H'}
# Posterior Predictive Check
pp_check(modelo_bayes)
```

A análise visual revela um desfasamento entre os valores observados e as distribuições simuladas pelo modelo. A linha dos dados reais não coincide com a zona de maior densidade das previsões, indicando que o modelo não reproduz totalmente o padrão dos dados. Assim, o ajustamento é apenas moderado e tem que ser ajustado.

\newpage

# Seleção de Modelos

Para a avaliação do modelo Bayesiano, utilizou-se o critério LOO-CV (Leave-One-Out Cross-Validation), que avalia a capacidade preditiva fora da amostra através do ELPD (Expected Log Predictive Density). Este critério é particularmente adequado em inferência Bayesiana e revela-se mais robusto do que alternativas como o WAIC, sobretudo em modelos com maior complexidade.

## Modelos Comparados

Foram considerados três modelos alternativos, cada um sem uma das variáveis mão explicativas e outro apenas com aquela que considerámos mais explicativa do modelo:

- **Modelo 1**, que inclui todas as variáveis explicativas selecionadas no estudo (PIB real, proporção de jovens, emprego no setor terciário e rendimento médio líquido);

- **Modelo 2**, que inclui todas as variáveis explicativas menos o PIB_real.

- **Modelo 3**, que inclui todas as variáveis explicativas menos a Proporção_15_24.

- **Modelo 4**, que inclui todas as variáveis explicativas menos a Rendimento_medio_liquido .

- **Modelo 5**, que inclui apenas a variável com maior relevância posterior identificada na análise inicial, sendo essa o setor terciário.

Ambos os modelos foram estimados com o mesmo número de cadeias, iterações, prioris e algoritmo MCMC, garantindo condições comparáveis.
...
Os resultados indicam que o modelo_4 apresentou o maior ELPD, mas os modelos modelo_2, modelo_bayes e modelo_3 tiveram desempenho muito semelhante, com diferenças pequenas ou não significativas. Já o modelo_5 mostrou desempenho claramente inferior.
A análise dos coeficientes posteriors do modelo_bayes indicou que apenas Emprego_Terciario tem efeito robusto sobre a taxa de desemprego, enquanto PIB_real, Proporção_15_24 e Rendimento_medio_liquido não apresentaram efeito significativo

## Modelo Selecionado

O modelo escolhido é o modelo_4, pois é o de referência com melhor elpd, outras combinações não trazem melhorias significativas (ou são levemente piores). O modelo_2 poderia ser considerado se quisesse um modelo mais simples, já que a diferença não é estatisticamente significativa, mas modelo_4 é mais completo.
...
```{r fig12, echo=FALSE, fig.cap="Gráfico da Previsão de Bayes"}
...
**Livro Bayes Rules** de <https://www.bayesrulesbook.com/chapter-9>
